...
const trustedSources = [
  ...
  [/Valor Econômico/i, .94], [/Nexo Jornal/i, .94], [/Agência Brasil/i, .92], [/Crux/i, .92],
...
async function fetchNews(search) {
  ...
  return [...xml.matchAll(/<item>([\s\S]*?)<\/item>/gi)].slice(0, 8).map((match, index) => {
    ...
    return { id: `news-${search.category}-${search.gl}-${index}-${Date.parse(publishedAt)}`, title, source, url: valueOf(block, "link"), category: search.category, publishedAt, quality: sourceQuality(source) };
