...
export default function AuthGate({ children }: { children: ReactNode }) {
  ...
  async function loadProfile(userId: string) {
    if (!supabase) return;
    setProfileError("");
    const { data, error } = await supabase.from("profiles").select("id,email,username,display_name,role").eq("id", userId).single();
    if (error) {
      setProfile(null);
  ...
  if (!session) return <main className="auth-page"><section className="auth-story"><a className="brand" href={`${BASE_PATH}/`}><span className="brand-mark">C</span><strong>Clarity</strong></a><div><p className="eyebrow">APRENDER SEM SER CONSUMIDO PELO FEED</p><h1>Seu tempo merece conteúdo com propósito.</h1><p>Vídeos longos, leituras, notícias e sínteses guiadas em uma experiência sem Shorts e sem rolagem infinita.</p><ul><li>Acesso contínuo para aprender</li><li>Algoritmo ajustado por você</li><li>Resumos que viram memória de longo prazo</li></ul></div><small>Clarity · formação guiada por intenção</small></section><section className="auth-panel">
    ...
    {mode === "verify" && <div className="auth-confirm"><span>✉</span><p className="eyebrow">CONFIRME SEU E-MAIL</p><h2>Um último passo</h2><p>Enviamos um link para <b>{message}</b>. Depois da confirmação, sua conta estará pronta.</p><button onClick={() => { setMode("signin"); setMessage(""); }}>Voltar para entrar</button></div>}
  </section></main>;
