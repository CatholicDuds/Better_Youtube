...
Depois dessa triagem, `scripts/audit-content.mjs` executa a avaliação decisiva sobre o conteúdo em si. O job lê legendas dos vídeos, o texto das notícias e transcrições publicadas dos episódios de podcast; materiais sem conteúdo suficiente não são aprovados. A auditoria semântica usa saída estruturada para pontuar importância, profundidade, insights, evidências, clareza, capacidade de prender atenção sem manipulação, substância e embalagem enganosa. A identidade da fonte é omitida do avaliador. Os resultados ficam em `public/data/content-audits.json`, são reutilizados nas execuções seguintes e somente itens com `approved: true` entram nos três feeds.
