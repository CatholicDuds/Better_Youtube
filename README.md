...
Para ativar a auditoria profunda gratuitamente, crie uma chave no Groq Console e salve-a no GitHub como o segredo `GROQ_API_KEY`. Opcionalmente, configure `GROQ_EVALUATION_MODEL`, `GROQ_TRANSCRIPTION_MODEL` e `CONTENT_AUDIT_BATCH_SIZE`. O padrão usa `openai/gpt-oss-20b` para avaliar o conteúdo e `whisper-large-v3-turbo` para transcrever apenas quando não há legenda ou transcrição publicada. As chamadas são serializadas, respeitam `Retry-After` e usam amostras distribuídas pela obra para caber no limite gratuito de tokens sem avaliar apenas a introdução. O workflow preserva as auditorias entre execuções para avançar pela fila sem gastar cota avaliando novamente o mesmo conteúdo. Sem a chave, nenhum material novo recebe aprovação semântica.
