...
Para ativar a auditoria profunda gratuitamente, crie uma chave no Groq Console e salve-a no GitHub como o segredo `GROQ_API_KEY`. Opcionalmente, configure `GROQ_EVALUATION_MODEL`, `GROQ_TRANSCRIPTION_MODEL`, `GROQ_AUDIT_INTERVAL_MS` e `CONTENT_AUDIT_BATCH_SIZE`. O padrão usa `openai/gpt-oss-20b` no endpoint estável de Chat Completions para avaliar o conteúdo e `whisper-large-v3-turbo` para transcrever apenas quando não há legenda ou transcrição publicada. As avaliações são espaçadas em 75 segundos, respeitam `Retry-After` e usam amostras distribuídas pela obra para caber no limite gratuito de tokens sem avaliar apenas a introdução. Se o modo JSON estrito falhar, uma tentativa posterior usa JSON mode e valida o contrato localmente. Podcasts longos usam até quatro amostras de cinco minutos distribuídas pela duração — vinte minutos de áudio no máximo por episódio. A coleta e a publicação continuam horárias; a auditoria programada roda a cada duas horas com um vídeo e uma notícia por rodada, enquanto podcasts entram a cada oito horas para preservar a cota de áudio. Push e execução manual auditam imediatamente. Ao confirmar um limite, o disjuntor encerra a rodada e preserva a fila para a próxima janela. O workflow preserva as auditorias entre execuções para avançar pela fila sem gastar cota avaliando novamente o mesmo conteúdo. Sem a chave, nenhum material novo recebe aprovação semântica.
